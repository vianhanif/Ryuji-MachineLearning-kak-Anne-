...
public class Model {
     ...
     * @param symbols the list of symbols.
     */
    public void train(List<Symbol> symbols) {
    ...
    protected double calculateInformation(List<Symbol> reply, List<Symbol> userKeywords) {
        ...
        for (Symbol aReply : reply) {           
            //System.out.println("reply: "+aReply.toString()+"\t"+userKeywords.contains(aReply));
            //System.out.println("reply: "+aReply.toString()+"\t"+containsString(userKeywords, aReply));
            if (userKeywords.contains(aReply)) {
                num++;
                //System.out.println("num: "+num+"\t"+aReply.toString());
            }
        }        
        if (num > 4) {
    ...
    public boolean containsString(List<Symbol> list, Symbol symbol){
        boolean result = false;

        for (int i=0; i<list.size();i++) {
            //System.out.println("list0: "+list.get(i).toString());
            //System.out.println("list1: "+symbol.toString());
            if (list.get(i).toString().equalsIgnoreCase(symbol.toString())){
                result = true;
            }
            break;            
        }        
    ...
    protected double calculateInformation(TrieNode trie, List<Symbol> reply, List<Symbol> userKeywords) {
        ...
        for (int i = 0; i < reply.size(); i++) {
            ...
            if (userKeywords.contains(symbol)) {    
                ...
                //System.out.println("lowerbond: "+lowerBound+"\t"+(i+1));
